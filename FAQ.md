...
本書では、GAMESS(US)「Ver. Aug 18, 2016」を使用して解説しております。「June 30, 2020 R1」以降のバージョンは、MoCalc2012(ver4.2.0.3)と上手く連携・実行できないようなので推奨しません。「June 30, 2020 R1」以降のバージョンは、それ以前のバージョンから特に初心者の方が扱うような新しい機能などは追加されていません。  

本書を読了後、最新のバージョンの[変更履歴](https://www.msg.chem.iastate.edu/gamess/versions.html)を確認し、ご自身の業務にどうしても必要な場合などに導入をご検討ください。初心者の方でよくあるのが、とにかく最新のバージョンを使いたがる事です。専門家でないかぎり、最新のものはバグなどを含む可能性があるので、できるだけ安定したバージョンを選ぶようにしましょう。
